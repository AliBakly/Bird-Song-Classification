function [Xmat,Tmat]=syllablecut(data,fs,Filtl,Filts,minsp,maxsp,minlength);
...
for i=1:length(diff(sub))
    ...
    if (max(in)- min(in))/fs> minlength/1000
        if figoff==0
        ...
        xx=data(in);
        tt=in;
        Xmat(1:length(xx),temp)=xx;
        Tmat(1:length(tt),temp)=tt/fs;
        temp=temp+1;
    end
end
